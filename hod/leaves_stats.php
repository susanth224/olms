...
if(isset($_SESSION['hod']) && isset($_SESSION['branch']))
{
	...
	<table cellpadding="0" cellspacing="0" border="0" class="table" >
		...
		$todaydate=date("j m Y",(mktime(date("j"),date("m"),date("Y"))));
											$res=mysql_query("SELECT count(*) FROM leaves WHERE `branch`='$branch' AND `adate`='$todaydate'");
											while($row=mysql_fetch_array($res)){$tdtl=$row[0];}
											$res=mysql_query("SELECT count(*) FROM leaves WHERE `branch`='$branch' AND `adate`='$todaydate' AND permission='wait' ");
											while($row=mysql_fetch_array($res)){$tdrqst=$row[0];}
											$res=mysql_query("SELECT count(*) FROM leaves WHERE `branch`='$branch' AND `adate`='$todaydate' AND permission='granted'");
											while($row=mysql_fetch_array($res)){$tdsncn=$row[0];}
											$res=mysql_query("SELECT count(*) FROM leaves WHERE `branch`='$branch' AND `adate`='$todaydate' AND permission='refuse'");
											while($row=mysql_fetch_array($res)){$tdunsncn=$row[0];}
											$res=mysql_query("SELECT count(*) FROM leaves WHERE `branch`='$branch' AND ogtime!='--' AND ictime='--' AND permission='granted'");
											while($row=mysql_fetch_array($res)){$lvout=$row[0];
											?>
