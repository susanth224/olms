...
$id=htmlspecialchars(addslashes($_SESSION["id"]));
$od=mysql_query("select * from outing where id='$id' order by sno desc;")or die(mysql_error());
...
<p style="text-align:center"><a href="#" class="css-tooltip-right color-blue"><img src="get.php?id=<?php echo $id;?>" style="width:87%;height:40%;border:2px solid green;padding:1px;"><span><?php echo "Me, ".$st_det['name']."<br>".$st_det['branch']." Department." ;?></span></a><br/><font size='3' face='Serif' >
			<ul class="olms4" style="">
                     ...
                     <li id="outings_act"  class="active"><a href="#" class="olms1" id="outingl" onclick="getoutingrs()">
					OUTINGS<?php
					$ods=mysql_query("select * from outing where id='$id' order by odate asc;")or die(mysql_error());
					$odata_che=mysql_fetch_array($ods);
					for($i=0;$i<mysql_num_rows($ods);$i++)
					{
						$odata=mysql_fetch_array($ods);
					...
					LEAVES<?php
					$odl=mysql_query("select * from leaves where id='$id' order by odate asc;")or die(mysql_error());
					for($i=0;$i<mysql_num_rows($odl);$i++)
					{
						$odata=mysql_fetch_array($odl);
						if( $odata["permission"]=="granted" && $odata["ogtime"]=="--")
						{
							echo "<img src='images/info1.gif'>";break;
						}
			...
			<li ><a href="changepass.php?id=<?php echo $id;?>" rel='facebox' class="olms1">CHANGE PASSWORD</a></li>
						...
						<table cellpadding="0" cellspacing="0" width="85%" class="sortable" style="padding-left:10%;font-size:12;">
							...
							<thead>
<tr style="color:white;background-image:url('images/stats.jpg');"><th colspan="7" style="text-align:center;"><?php echo $id ?> - OUTINGS</th></tr>
								<tr style="background-color:#CCCCCC;" class="stretchRight">
									<th>Permission</th>
									<th>Date Outing</th>
									<th>Created date</th>
									<th>Gone_time</th>
...
?>
				...
				$ic="--";
				 ...
				 {
					 echo $appro?>&nbsp;&nbsp; <a href="cancel-outing.php?sno=<?php echo $odata['sno'];?>">Cancel</a><?php
				 }
					 else
					 {
						echo $permiss;
				...
				<td><?php echo $odata['adate']; ?></td>
				<td><?php echo $odata['ogtime']; ?></td>
				<td><?php echo $odata['ictime']; ?></td>
				<?php 
				...
				{
				?>
				<td><a  rel="facebox" title="Upload Supported Documents" href="outing_proof.php?sno=<?php echo $odata['sno'];?> ">Upload</a></td>
				<?php
				}
				else
				{
				?>
				<td>Already Uploaded<br><a  rel="facebox" href="outing_proof.php?sno=<?php echo $odata['sno'];?>">Re-Upload</a>&nbsp;
				<?php 
			  echo  "<blink><a  style='color:green;' onclick=\"file(".$odata["sno"].")\" '>Open</a></blink>"; ?></td>
			<?php	}
			}
			else
			{
			...
			<script>
					function file(sno)
	{
	window.open("file_open_outing.php?sno="+sno," ","left=400,width=650,height=650");
	}
	</script>
			</tr>	
<?php
