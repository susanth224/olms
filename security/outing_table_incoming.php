	...
	<?php include('modal_delete.php'); ?>
		...
		<tbody>
		<?php
					$res=mysql_query("select * from outing where permission='granted' and icdate='--' and ogtime!='--';");
					...
					<td class="empty" ="160">
		<a data-placement="top" title="Click to Approve" id="approve<?php echo $row['sno']; ?>" href='oic.php?sno=<?php echo "".$row['sno']."";?>' class="btn btn-success"><i class="icon-ok icon-large"></i>Report</a></a>
			<script type="text/javascript">
			$(document).ready(function(){
				$('#oic<?php echo  $row['sno']; ?>').tooltip('show');
				$('#oic<?php echo  $row['sno']; ?>').tooltip('hide');
			});
			</script>
		</td> 
		</tr>
