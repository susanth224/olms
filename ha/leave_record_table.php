	...
	//	$resul=mysql_query("SELECT * FROM data WHERE id='$id'");
		...
		//	{?>
						...
						<div style='border:1px solid #ddd;border:1px solid #ddd;padding:10px;height:px;width:460px;overflow:;'>
								...
								$res=mysql_query("SELECT count(*) FROM leaves WHERE permission='granted' and ogdate='".$date."'");
								while($row=mysql_fetch_array($res)){$tog=$row[0];}  ?>
								<tr class="info"><td colspan="4"> <b >Outgoing STATUS ON <?php echo $date;?>&ndash;&ndash;&ndash;&ndash;&ndash;&ndash;&ndash;&ndash;&ndash;&ndash;&ndash;&ndash;&ndash;&ndash;&ndash;&ndash;&ndash;&ndash;&ndash;&ndash;&ndash;&ndash;&ndash;TOTAL-<?php echo $tog;?>
					 </b></td></tr>
								<tr>
									<th width="5%">Sno</th>
									<th width="10%">ID</th>
									<th width="8%">Outgoing Date</th>
									<th width="10%">Outgoing  Time</th>
								</tr>
						<?php
							$i=0;
					$res=mysql_query("SELECT * FROM leaves WHERE block='".$block."' AND permission='granted' and ogdate='".$date."'");
						...
						<div style='border:1px solid #ddd;border:1px solid #ddd;padding:10px;height:px;width:455px;overflow:;'>
									<table style="font-size: 12px;" class='table'>			<?php  $todaydate=date("d m Y");$wai='wait';
								$res=mysql_query("SELECT count(*) FROM leaves WHERE permission='granted' and icdate='".$date."'");
								...
								<tr>
									<th width="5%">Sno</th>
									<th width="10%">ID</th>
									<th width="8%">Incoming Date</th>
									<th width="10%">Incoming Time</th>
								</tr>
							<?php
							$i=0;
					$res=mysql_query("SELECT * FROM leaves WHERE block='".$block."' AND permission='granted' and icdate='".$date."'");
							while ($row=mysql_fetch_array($res))
							{
							...
							$i++;?>
									...
									<td width="3%"><?php echo "$i";?></td>
									<td width="10%"><?php echo "".$row['id']."";?></td>
									<td width="10%"><?php echo "".$row['icdate']."";?></td>
