...
<table>
				...
				<?php
				$res=mysql_query("SELECT count(*) FROM outing WHERE `block`='$block' AND `id`='$user' AND permission='granted'");
											while($row=mysql_fetch_array($res)){$tout=$row[0];}
											$res=mysql_query("SELECT count(*) FROM leaves WHERE `block`='$block' AND `id`='$user' AND permission='granted'");
	...
	$.post("studetails.php?id="+id,function(id)
		{
			...
			echo"<h1>THIS ID NOT BELONGS TO $block</h1>";
		?>
