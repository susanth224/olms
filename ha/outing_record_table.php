...
if (isset($_GET['date']))
	...
	//	$resul=mysql_query("SELECT * FROM data WHERE id='$id'");
		...
		//	{?>
						...
						<?php
							$i=0;
					$res=mysql_query("SELECT * FROM outing WHERE block='".$block."' AND permission='granted' and ogdate='".$date."'");
						...
						<div style='border:1px solid #ddd;border:1px solid #ddd;padding:10px;height:px;width:455px;overflow:;'>
							...
							<?php
							$i=0;
					$res=mysql_query("SELECT * FROM outing WHERE block='".$block."' AND permission='granted' and icdate='".$date."'");
							while ($row=mysql_fetch_array($res))
							{
