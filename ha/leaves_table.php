	...
	<?php include('modal_delete.php'); ?>
		...
		<tbody>
		<?php
					$res=mysql_query("SELECT * FROM leaves WHERE block='".$block."' AND permission='wait' ORDER BY sno ASC ");
							...
							{		echo "<tr>"?>
								...
								<td class="empty" ="190">
		<a data-placement="top" title="Click to Approve" id="approve<?php echo $row['sno']; ?>" href='approve_leaves.php?sno=<?php echo "".$row['sno']."";?>' class="btn btn-success"><i class="icon-ok icon-large"></i>Approve</a>
		...
		<a data-placement="top" title="Click to Refuse" id="reject<?php echo $row['sno']; ?>" href='refuse_leaves.php?sno=<?php echo "".$row['sno']."";?>' class="btn btn-warning"><i class="icon-remove icon-large"></i>Refuse</a>
			<script type="text/javascript">
			$(document).ready(function(){	
			...
			<?php
			 ...
			 { echo "No file found";
			  ...
			  {
			  echo  "<blink><a style='font-size:15px;color:black;text-decoration:underline;' onclick=\"file(".$row["sno"].")\"  title='".$row["comment"]."'>View New file</a></blink>";
			  }
			  else
			  {
			  echo "No file Found";
