	...
	while($row=mysql_fetch_array($result))
	{
		...
		$resul=mysql_query("SELECT * FROM users WHERE id='$id'");
			while($row=mysql_fetch_array($resul))
			{
				...
				<div style='border:1px solid #ddd;border:1px solid #ddd;padding:10px;height:200;width:400px;overflow:; text-align: left;'>
				...
				<div style='border:1px solid #ddd;border:1px solid #ddd;padding:10px;height:230px;width:175px;overflow:;'>
				<ul class="thumbnails">
					<li class="span3" style="width:170px;height:170px;">
					  <a class="thumbnail">
	
						<img src="get.php?id=<?php echo $id;?>" style="width:99%;height:180px;"><br/><font size='3' face='Serif' ></a>

					  </a>
					</li>
					</ul>
					<?php
						$res=mysql_query("SELECT count(*) FROM outing WHERE `id`='$id' AND `permission`='granted' ");
						while($row=mysql_fetch_array($res)){$leaves=$row[0];}
						?>
						<BR><br>
					<h4>Total Outings:<?php echo $leaves;?></h4>
					<h4><?php //echo $outings;?></h4>
				</div>
				</td>
				</tr>
				<tr>
			...
			<table>
				...
				<table>
					<tr style="font-color:red;"  >Total LEAVES STATUS</h5></tr>
				<tr>
									<th width="3%">Sno</th>
				...
				<table >
								<?php
							$i=0;
					$res=mysql_query("SELECT * FROM outing WHERE id='".$id."' AND `permission`='granted'");
