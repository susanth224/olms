...
if(isset($_SESSION['office']))
{
	...
	<table cellpadding="0" cellspacing="0" border="0" class="table" >
		...
		$todaydate=date("j m Y",(mktime(date("j"),date("m"),date("Y"))));
											$res=mysql_query("SELECT count(*) FROM leaves WHERE `adate`='$todaydate'");
											while($row=mysql_fetch_array($res)){$tdtl=$row[0];}
											$res=mysql_query("SELECT count(*) FROM leaves WHERE `adate`='$todaydate' AND permission='wait' ");
											...
											$res=mysql_query("SELECT count(*) FROM leaves WHERE `adate`='$todaydate' AND permission='refuse'");
											while($row=mysql_fetch_array($res)){$tdunsncn=$row[0];}
											$res=mysql_query("SELECT count(*) FROM leaves WHERE ogtime!='--' AND ictime='--' AND permission='granted'");
